...
abstract class WorkshopUnity
{
  ...
  static public function InitResult()
  {
    self::$Result = new Result();
  }

  static public function registerDelayFn(\Closure $fn): void
  {
    self::$listDelayFn[] = $fn;
  }

  static public function callDelayFn(): void
  {
...
final class CarolinaPropertyFilter implements PropertyFilter
{
  ...
  static public function ManualFields($key = null, $list = false): mixed
  {
    $filter = [
      'CML2_ARTICLE' => true,
      'TSVET' => true,
...
final class WorkshopIBlockElement extends WorkshopUnity
{
  ...
  static private function _giveXmlId(?FullDataElement $FullDataElementMain, FullDataElement $FullDataElementRoot, $heir): string
  {
    if ($heir) {
  ...
  static private function _CanonocalValuePropLine(array $FL, array &$linked): void
  {
  ...
  public function CopyElement(?FullDataElement $FullDataElementMain = null, $heir = false): Result
  {
    if ($FullDataElementMain)
      ...
      $this->setFullDataElementMain($this->getFullDataElementRoot());
  ...
  public function CopyElementProperty(): Result
  {
    ...
    if (empty($elementPropsRoot)) return self::_ErrorGenerator('[#WIBE-B002] Свойства элемента ' . $elementIdRoot . ' не найдены');
