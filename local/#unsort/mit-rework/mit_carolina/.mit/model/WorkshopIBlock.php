...
const ARTICLE_CODE = 'CML2_ARTICLE';
...
final class WorkshopIBlockElementCollector
{
    ...
    public function CollectorDataInfo($name = null): array
    {
        $Result = \Bitrix\Iblock\ElementTable::getList([
            'select' => ['COUNT'],
            'filter' => ['IBLOCK_ID' => $this->getIBlockIdRoot(), '>=COUNT' => \MIT\Shell\EX1C_CLONE_NAME_COUNT, ...($name ? ['NAME' => $name] : [])],
            'group' => ['NAME'],
            'runtime' => array(
    ...
    public function CollectorData(): \Generator
    {
        ...
        && $Result = \Bitrix\Iblock\ElementTable::getList([
            'select' => ['NAME', 'COUNT'],
            'filter' => ['IBLOCK_ID' => $this->getIBlockIdRoot(), '>=COUNT' => \MIT\Shell\EX1C_CLONE_NAME_COUNT],
            'group' => ['NAME'],
            'runtime' => array(
                new \Bitrix\Main\Entity\ExpressionField('COUNT', 'COUNT(*)'),
            ),
    ...
    public function YavlyaetsyaBazovoy(int $id): bool
    {
       return ((new \Bitrix\Main\Entity\Query(\Bitrix\Iblock\ElementPropertyTable::getEntity()))
        ->registerRuntimeField(
          'ELEMENT',
          [
        ...
        ->setFilter([
          ...
          '=PROPERTY.CODE' => 'YAVLYAETSYABAZOVOY'
        ])
        ->exec()
        ->fetch()['VALUE'] === 'true');
    }
    ...
    public function FullData(string $elementName): \Generator
    {
        ...
        do {
            if ($row = $Result->fetch()) {
                $row['YAVLYAETSYABAZOVOY'] = $this->YavlyaetsyaBazovoy($row['ID']);
...
final class WorkshopIBlockElement extends WorkshopUnity
{
    ...
    public function CheckChangeElement_D($time = 0): WorkshopIBlockElement
    {
        ...
        if ($rowRoot['TIMESTAMP_X']->getTimestamp() <= $time) {
            if ($rowRoot['ACTIVE'] === 'N') {
                throw new \Exception("ElementRoot [R] #$elementIdRoot not changed by timestamp", -3);
            } else {
                throw new \Exception("ElementRoot [Rd] #$elementIdRoot not changed by timestamp", -2);
            }
        }
    ...
    public function checkMountElement_D(int $iblockIdTarget, string $elementXMLRoot){
      return $this->checkMountElement($iblockIdTarget, $elementXMLRoot, [
        'FILTER_XML' => $elementXMLRoot,
        'SKIP_COND'  => true
      ]);
    }

    public function checkMountElement(int $iblockIdTarget, string $elementXMLRoot, array $extend = []): array
    {
        static $self_rowTargetMount = null;

        $_do = !($extend['SKIP_COND'] ?? false);

        if ($_do && (($self_rowTargetMount['ID'] ?? false) === $iblockIdTarget)) {
        ...
        $rowTargetMount = \Bitrix\Iblock\ElementTable::getList([
            ...
            'filter' => [
              'IBLOCK_ID' => $iblockIdTarget,
              'XML_ID' => $extend['FILTER_XML'] ?? ('%#' . $elementXMLRoot)
            ],
            'limit' => 1,
        ])->fetch();
        ...
        if($_do){
          ...
          return ($rowTargetMount ? $rowTargetMount : []);
        }
    ...
    public function CopyElement_D(&$FullDataElementTarget): Result
    {
        $FullDataElementRoot = $this->getFullDataElementRoot();
        $iblockIdTarget = $this->getIBlockIdRoot();

        $FullDataElementTarget = clone $FullDataElementRoot;

        $FullDataElementTarget->XML_ID = 'D!' . $FullDataElementRoot->XML_ID;
        $FullDataElementTarget->ACTIVE = 'N';

        unset(
            $FullDataElementTarget->ID,
            $FullDataElementTarget->TMP_ID,
            $FullDataElementTarget->EXTERNAL_ID,
            $FullDataElementTarget->WF_LAST_HISTORY_ID,
            $FullDataElementTarget->YAVLYAETSYABAZOVOY,
        );

        $elementTarget = new \CIBlockElement();

        if ($FullDataElementTarget->ID = ($this->checkMountElement_D($iblockIdTarget, $FullDataElementTarget->XML_ID)['ID'] ?? 0)) {
            unset( $FullDataElementTarget->CODE);
            $elementTarget->Update($FullDataElementTarget->ID, $FullDataElementTarget());
        } else {
            $FullDataElementTarget->CODE = md5($FullDataElementRoot->ID . $FullDataElementRoot->CODE . GUIDv4());
            $FullDataElementTarget->ID = (int) $elementTarget->Add($FullDataElementTarget());
        }

        if (!$FullDataElementTarget->ID || $elementTarget->LAST_ERROR)
            return self::_ErrorGenerator('[#WIBE-A001] Ошибка при копировании элемента: ' . $elementTarget->LAST_ERROR);

        return self::$Result;
    }

    public function CopyElementProperty(?FullDataElement $FullDataElementTarget = null): Result
    {
        global $APPLICATION;

        $FullDataElementTarget = $FullDataElementTarget ?? $this->getFullDataElementTarget();

        $iblockIdRoot = $this->getIBlockIdRoot();
        $iblockIdTarget = $FullDataElementTarget->IBLOCK_ID;
    ...
    public function CopyElementProperty_D($FullDataElementTarget): Result
    {
      return $this->CopyElementProperty($FullDataElementTarget);
    }
